...
- SDK 会利用服务器返回的 `timestamp` 与请求往返时间，持续估计“服务器时间 - 本地时间”的偏移。
- `waitForNextRoundWindow()` 会按“估计的服务器当前时间”对齐，而不是死用本机系统时钟。
- 因此一般不再要求你手动把系统时间校到“几乎 0 误差”。

仍建议保证系统时间大致正确（例如开启 NTP），但不需要苛求毫秒级零偏差。
